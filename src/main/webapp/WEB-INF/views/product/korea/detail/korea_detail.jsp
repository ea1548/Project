...
<body>
	...
	<div class="info">
		...
		<form action="/CartAdd" method="Post">
			<input type="hidden" name="product_name" value="${koreaInfo.product_name}">
			<input type="hidden" name="product_price" value="${koreaInfo.product_price}">
			<input type="hidden" name="product_number" value="${koreaInfo.product_number}">			
    		...
    		function cartOrder(){
    			var result = confirm("${koreaInfo.product_price}  ${KoreaInfo.product_stock}원을 결제하시겠습니까?")
    				if(result){
    					alert("결제가 완료되었습니다.");
    					location.href = "<c:url value='/DeleteCart' />";
    				} else {
    					alert("결제가 취소되었습니다.");
    					location.href = "/product/korea/detail/korea_detail?product_number=" + "${koreaInfo.product_number}";
				...
				<textarea style="width: 675px; height: 200px;" readonly>${koreaInfo.product_desc}</textarea>
			</ul>
			 <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
		</form>
