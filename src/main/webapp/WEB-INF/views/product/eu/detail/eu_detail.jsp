...
<%@include file="/WEB-INF/views/include/nav.jsp" %>
	<div class="info">
		<div class="image_wrap"
			 data-filename="${euInfo.fileName}"
			 data-productnumber="${euInfo.product_number}"
			 data-path="${euInfo.uploadPath}"
			 data-uuid="${euInfo.uuid}">
			<img>
		</div>
		<form action="/CartAdd" method="Post">
			<input type="hidden" name="product_name" value="${euInfo.product_name}">
			<input type="hidden" name="product_price" value="${euInfo.product_price}">
			<input type="hidden" name="product_number" value="${euInfo.product_number}">		
		...
		<ul>
    		...
    		function cartOrder(){
    			var result = confirm("${euInfo.product_price}  ${euInfo.product_stock}원을 결제하시겠습니까?")
    				if(result){
    					alert("결제가 완료되었습니다.");
    					location.href = "<c:url value='/DeleteCart' />";
    				} else {
    					alert("결제가 취소되었습니다.");
    					location.href = "/product/eu/detail/eu_detail?product_number=" + "${euInfo.product_number}";
			...
			<textarea style="width: 675px; height: 200px;" readonly>${euInfo.product_desc}</textarea>
		</ul>
			<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
		</form>	
